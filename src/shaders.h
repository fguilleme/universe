...
static const char *kRingsVS =
    "#version 330 core\n"
    "layout(location=0) in vec3 aPos;\n"
    "layout(location=1) in float aOuter;\n"
    "layout(location=2) in float aInnerRatio;\n"
    ...
    "  vec3 ref = (abs(n.z) < 0.99) ? vec3(0.0, 0.0, 1.0) : vec3(0.0, 1.0, 0.0);\n"
    "  vec3 t = normalize(cross(n, ref));\n"
    "  vec3 b = cross(n, t);\n"
    "  vec3 worldPos = aPos + (t * (corner.x * aOuter)) + (b * (corner.y * aOuter));\n"
    "  gl_Position = uWorldToClip * vec4(worldPos, 1.0);\n"
    "}\n";

static const char *kRingsFS =
    "#version 330 core\n"
    "in vec2 vLocal;\n"
    "in float vInner;\n"
    "uniform sampler2D uRingTex;\n"
    "uniform int uRadialStrip;\n"
    "out vec4 oColor;\n"
    "void main(){\n"
    ...
    "  vec2 uv;\n"
    "  if (uRadialStrip == 1) {\n"
    "    float t = (r - vInner) / max(1e-6, (1.0 - vInner));\n"
    "    uv = vec2(clamp(t, 0.0, 1.0), 0.5);\n"
    "  } else {\n"
    "    uv = vLocal * 0.5 + 0.5;\n"
    "  }\n"
    "  vec4 tex = texture(uRingTex, uv);\n"
    "  if (tex.a < 0.01) discard;\n"
    "  float outer_fade = smoothstep(1.0, 0.98, r);\n"
    "  float inner_fade = smoothstep(vInner, vInner + 0.02, r);\n"
    "  tex.a *= outer_fade * inner_fade;\n"
    "  oColor = tex;\n"
    "}\n";
