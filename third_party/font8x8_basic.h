// Public domain 8x8 font (font8x8_basic).
// Source: https://github.com/dhepper/font8x8 (public domain).

#pragma once

#include <stdint.h>

// Each glyph is 8 bytes, one per row, LSB on the left.
static const uint8_t font8x8_basic[128][8] = {
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},  // U+0000 (nul)
    {0x7E, 0x81, 0xA5, 0x81, 0xBD, 0x99, 0x81, 0x7E},  // U+0001
    {0x7E, 0xFF, 0xDB, 0xFF, 0xC3, 0xE7, 0xFF, 0x7E},  // U+0002
    {0x6C, 0xFE, 0xFE, 0xFE, 0x7C, 0x38, 0x10, 0x00},  // U+0003
    {0x10, 0x38, 0x7C, 0xFE, 0x7C, 0x38, 0x10, 0x00},  // U+0004
    {0x38, 0x7C, 0x38, 0xFE, 0xFE, 0xD6, 0x10, 0x38},  // U+0005
    {0x10, 0x38, 0x7C, 0xFE, 0xFE, 0x7C, 0x10, 0x38},  // U+0006
    {0x00, 0x00, 0x18, 0x3C, 0x3C, 0x18, 0x00, 0x00},  // U+0007
    {0xFF, 0xFF, 0xE7, 0xC3, 0xC3, 0xE7, 0xFF, 0xFF},  // U+0008
    {0x00, 0x3C, 0x66, 0x42, 0x42, 0x66, 0x3C, 0x00},  // U+0009
    {0xFF, 0xC3, 0x99, 0xBD, 0xBD, 0x99, 0xC3, 0xFF},  // U+000A
    {0x0F, 0x07, 0x0F, 0x7D, 0xCC, 0xCC, 0xCC, 0x78},  // U+000B
    {0x3C, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x7E, 0x18},  // U+000C
    {0x3F, 0x33, 0x3F, 0x30, 0x30, 0x70, 0xF0, 0xE0},  // U+000D
    {0x7F, 0x63, 0x7F, 0x63, 0x63, 0x67, 0xE6, 0xC0},  // U+000E
    {0x99, 0x5A, 0x3C, 0xE7, 0xE7, 0x3C, 0x5A, 0x99},  // U+000F
    {0x80, 0xE0, 0xF8, 0xFE, 0xF8, 0xE0, 0x80, 0x00},  // U+0010
    {0x02, 0x0E, 0x3E, 0xFE, 0x3E, 0x0E, 0x02, 0x00},  // U+0011
    {0x18, 0x3C, 0x7E, 0x18, 0x18, 0x7E, 0x3C, 0x18},  // U+0012
    {0x66, 0x66, 0x66, 0x66, 0x66, 0x00, 0x66, 0x00},  // U+0013
    {0x7F, 0xDB, 0xDB, 0x7B, 0x1B, 0x1B, 0x1B, 0x00},  // U+0014
    {0x3E, 0x61, 0x3C, 0x66, 0x66, 0x3C, 0x86, 0x7C},  // U+0015
    {0x00, 0x00, 0x00, 0x00, 0x7E, 0x7E, 0x7E, 0x00},  // U+0016
    {0x18, 0x3C, 0x7E, 0x18, 0x7E, 0x3C, 0x18, 0xFF},  // U+0017
    {0x18, 0x3C, 0x7E, 0x18, 0x18, 0x18, 0x18, 0x00},  // U+0018
    {0x18, 0x18, 0x18, 0x18, 0x7E, 0x3C, 0x18, 0x00},  // U+0019
    {0x00, 0x18, 0x0C, 0xFE, 0x0C, 0x18, 0x00, 0x00},  // U+001A
    {0x00, 0x30, 0x60, 0xFE, 0x60, 0x30, 0x00, 0x00},  // U+001B
    {0x00, 0x00, 0xC0, 0xC0, 0xC0, 0xFE, 0x00, 0x00},  // U+001C
    {0x00, 0x24, 0x66, 0xFF, 0x66, 0x24, 0x00, 0x00},  // U+001D
    {0x00, 0x18, 0x3C, 0x7E, 0xFF, 0xFF, 0x00, 0x00},  // U+001E
    {0x00, 0xFF, 0xFF, 0x7E, 0x3C, 0x18, 0x00, 0x00},  // U+001F
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},  // U+0020 (space)
    {0x18, 0x3C, 0x3C, 0x18, 0x18, 0x00, 0x18, 0x00},  // U+0021 (!)
    {0x6C, 0x6C, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00},  // U+0022 (")
    {0x6C, 0x6C, 0xFE, 0x6C, 0xFE, 0x6C, 0x6C, 0x00},  // U+0023 (#)
    {0x18, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x18, 0x00},  // U+0024 ($)
    {0x00, 0xC6, 0xCC, 0x18, 0x30, 0x66, 0xC6, 0x00},  // U+0025 (%)
    {0x38, 0x6C, 0x38, 0x76, 0xDC, 0xCC, 0x76, 0x00},  // U+0026 (&)
    {0x30, 0x30, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00},  // U+0027 (')
    {0x0C, 0x18, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00},  // U+0028
    {0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x18, 0x30, 0x00},  // U+0029
    {0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00},  // U+002A
    {0x00, 0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x00},  // U+002B
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x30},  // U+002C
    {0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00},  // U+002D
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00},  // U+002E
    {0x06, 0x0C, 0x18, 0x30, 0x60, 0xC0, 0x80, 0x00},  // U+002F
    {0x7C, 0xC6, 0xCE, 0xD6, 0xE6, 0xC6, 0x7C, 0x00},  // U+0030
    {0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00},  // U+0031
    {0x7C, 0xC6, 0x0E, 0x1C, 0x70, 0xC0, 0xFE, 0x00},  // U+0032
    {0x7C, 0xC6, 0x06, 0x3C, 0x06, 0xC6, 0x7C, 0x00},  // U+0033
    {0x1C, 0x3C, 0x6C, 0xCC, 0xFE, 0x0C, 0x1E, 0x00},  // U+0034
    {0xFE, 0xC0, 0xFC, 0x06, 0x06, 0xC6, 0x7C, 0x00},  // U+0035
    {0x3C, 0x60, 0xC0, 0xFC, 0xC6, 0xC6, 0x7C, 0x00},  // U+0036
    {0xFE, 0xC6, 0x0C, 0x18, 0x30, 0x30, 0x30, 0x00},  // U+0037
    {0x7C, 0xC6, 0xC6, 0x7C, 0xC6, 0xC6, 0x7C, 0x00},  // U+0038
    {0x7C, 0xC6, 0xC6, 0x7E, 0x06, 0x0C, 0x78, 0x00},  // U+0039
    {0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x00},  // U+003A
    {0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x30},  // U+003B
    {0x0E, 0x1C, 0x38, 0x70, 0x38, 0x1C, 0x0E, 0x00},  // U+003C
    {0x00, 0x00, 0x7E, 0x00, 0x00, 0x7E, 0x00, 0x00},  // U+003D
    {0x70, 0x38, 0x1C, 0x0E, 0x1C, 0x38, 0x70, 0x00},  // U+003E
    {0x7C, 0xC6, 0x0E, 0x1C, 0x18, 0x00, 0x18, 0x00},  // U+003F
    {0x7C, 0xC6, 0xDE, 0xDE, 0xDE, 0xC0, 0x78, 0x00},  // U+0040
    {0x38, 0x6C, 0xC6, 0xFE, 0xC6, 0xC6, 0xC6, 0x00},  // U+0041
    {0xFC, 0x66, 0x66, 0x7C, 0x66, 0x66, 0xFC, 0x00},  // U+0042
    {0x3C, 0x66, 0xC0, 0xC0, 0xC0, 0x66, 0x3C, 0x00},  // U+0043
    {0xF8, 0x6C, 0x66, 0x66, 0x66, 0x6C, 0xF8, 0x00},  // U+0044
    {0xFE, 0x62, 0x68, 0x78, 0x68, 0x62, 0xFE, 0x00},  // U+0045
    {0xFE, 0x62, 0x68, 0x78, 0x68, 0x60, 0xF0, 0x00},  // U+0046
    {0x3C, 0x66, 0xC0, 0xC0, 0xCE, 0x66, 0x3E, 0x00},  // U+0047
    {0xC6, 0xC6, 0xC6, 0xFE, 0xC6, 0xC6, 0xC6, 0x00},  // U+0048
    {0x3C, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00},  // U+0049
    {0x1E, 0x0C, 0x0C, 0x0C, 0xCC, 0xCC, 0x78, 0x00},  // U+004A
    {0xE6, 0x66, 0x6C, 0x78, 0x6C, 0x66, 0xE6, 0x00},  // U+004B
    {0xF0, 0x60, 0x60, 0x60, 0x62, 0x66, 0xFE, 0x00},  // U+004C
    {0xC6, 0xEE, 0xFE, 0xFE, 0xD6, 0xC6, 0xC6, 0x00},  // U+004D
    {0xC6, 0xE6, 0xF6, 0xDE, 0xCE, 0xC6, 0xC6, 0x00},  // U+004E
    {0x38, 0x6C, 0xC6, 0xC6, 0xC6, 0x6C, 0x38, 0x00},  // U+004F
    {0xFC, 0x66, 0x66, 0x7C, 0x60, 0x60, 0xF0, 0x00},  // U+0050
    {0x38, 0x6C, 0xC6, 0xC6, 0xDA, 0xCC, 0x76, 0x00},  // U+0051
    {0xFC, 0x66, 0x66, 0x7C, 0x6C, 0x66, 0xE6, 0x00},  // U+0052
    {0x7C, 0xC6, 0x60, 0x38, 0x0C, 0xC6, 0x7C, 0x00},  // U+0053
    {0x7E, 0x7E, 0x5A, 0x18, 0x18, 0x18, 0x3C, 0x00},  // U+0054
    {0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0x7C, 0x00},  // U+0055
    {0xC6, 0xC6, 0xC6, 0xC6, 0xC6, 0x6C, 0x38, 0x00},  // U+0056
    {0xC6, 0xC6, 0xC6, 0xD6, 0xFE, 0xEE, 0xC6, 0x00},  // U+0057
    {0xC6, 0xC6, 0x6C, 0x38, 0x38, 0x6C, 0xC6, 0x00},  // U+0058
    {0x66, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x3C, 0x00},  // U+0059
    {0xFE, 0xC6, 0x8C, 0x18, 0x32, 0x66, 0xFE, 0x00},  // U+005A
    {0x78, 0x60, 0x60, 0x60, 0x60, 0x60, 0x78, 0x00},  // U+005B
    {0xC0, 0x60, 0x30, 0x18, 0x0C, 0x06, 0x02, 0x00},  // U+005C
    {0x78, 0x18, 0x18, 0x18, 0x18, 0x18, 0x78, 0x00},  // U+005D
    {0x10, 0x38, 0x6C, 0xC6, 0x00, 0x00, 0x00, 0x00},  // U+005E
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF},  // U+005F
    {0x30, 0x18, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00},  // U+0060
    {0x00, 0x00, 0x78, 0x0C, 0x7C, 0xCC, 0x76, 0x00},  // U+0061
    {0xE0, 0x60, 0x60, 0x7C, 0x66, 0x66, 0xDC, 0x00},  // U+0062
    {0x00, 0x00, 0x7C, 0xC6, 0xC0, 0xC6, 0x7C, 0x00},  // U+0063
    {0x1C, 0x0C, 0x0C, 0x7C, 0xCC, 0xCC, 0x76, 0x00},  // U+0064
    {0x00, 0x00, 0x7C, 0xC6, 0xFE, 0xC0, 0x7C, 0x00},  // U+0065
    {0x3C, 0x66, 0x60, 0xF8, 0x60, 0x60, 0xF0, 0x00},  // U+0066
    {0x00, 0x00, 0x76, 0xCC, 0xCC, 0x7C, 0x0C, 0xF8},  // U+0067
    {0xE0, 0x60, 0x6C, 0x76, 0x66, 0x66, 0xE6, 0x00},  // U+0068
    {0x18, 0x00, 0x38, 0x18, 0x18, 0x18, 0x3C, 0x00},  // U+0069
    {0x06, 0x00, 0x06, 0x06, 0x06, 0x66, 0x66, 0x3C},  // U+006A
    {0xE0, 0x60, 0x66, 0x6C, 0x78, 0x6C, 0xE6, 0x00},  // U+006B
    {0x38, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00},  // U+006C
    {0x00, 0x00, 0xCC, 0xFE, 0xFE, 0xD6, 0xC6, 0x00},  // U+006D
    {0x00, 0x00, 0xDC, 0x66, 0x66, 0x66, 0x66, 0x00},  // U+006E
    {0x00, 0x00, 0x7C, 0xC6, 0xC6, 0xC6, 0x7C, 0x00},  // U+006F
    {0x00, 0x00, 0xDC, 0x66, 0x66, 0x7C, 0x60, 0xF0},  // U+0070
    {0x00, 0x00, 0x76, 0xCC, 0xCC, 0x7C, 0x0C, 0x1E},  // U+0071
    {0x00, 0x00, 0xDC, 0x76, 0x66, 0x60, 0xF0, 0x00},  // U+0072
    {0x00, 0x00, 0x7E, 0xC0, 0x7C, 0x06, 0xFC, 0x00},  // U+0073
    {0x30, 0x30, 0xFC, 0x30, 0x30, 0x36, 0x1C, 0x00},  // U+0074
    {0x00, 0x00, 0xCC, 0xCC, 0xCC, 0xCC, 0x76, 0x00},  // U+0075
    {0x00, 0x00, 0xC6, 0xC6, 0xC6, 0x6C, 0x38, 0x00},  // U+0076
    {0x00, 0x00, 0xC6, 0xD6, 0xFE, 0xFE, 0x6C, 0x00},  // U+0077
    {0x00, 0x00, 0xC6, 0x6C, 0x38, 0x6C, 0xC6, 0x00},  // U+0078
    {0x00, 0x00, 0xC6, 0xC6, 0xC6, 0x7E, 0x06, 0xFC},  // U+0079
    {0x00, 0x00, 0xFE, 0x4C, 0x18, 0x32, 0xFE, 0x00},  // U+007A
    {0x0E, 0x18, 0x18, 0x70, 0x18, 0x18, 0x0E, 0x00},  // U+007B
    {0x18, 0x18, 0x18, 0x00, 0x18, 0x18, 0x18, 0x00},  // U+007C
    {0x70, 0x18, 0x18, 0x0E, 0x18, 0x18, 0x70, 0x00},  // U+007D
    {0x76, 0xDC, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},  // U+007E
    {0x00, 0x10, 0x38, 0x6C, 0xC6, 0xC6, 0xFE, 0x00},  // U+007F
};

